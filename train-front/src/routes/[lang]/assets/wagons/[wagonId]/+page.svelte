<script lang="ts">
  ...
  $effect(() => {
    serialNumber.input = wagon.serialNumber
    modelName.input = wagon.model.name
    gauge.input = wagon.model.gauge
    modelTypeId.input = wagon.model.id
  })

  const decomission = async () => {
    await DecommissionWagonUseCaseApi.execute({ requestBody: { id: page.params.wagonId } })
    invalidateAll()
  }
</script>
